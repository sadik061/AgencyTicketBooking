...
class InsertDetails{
    function startInsertDetails(){
        ...
        try{
            if(isset($id)){
                $sqlInsert = "Delete From maindata WHERE maindata_id='$id'";
                $conn->exec($sqlInsert);
        ...
        if($sqlInsert){
            //success inserted
            echo '<script type="text/javascript">alert("Successfully Deleted Main Data");</script>';
        }else{
            echo '<script type="text/javascript">alert("Unable Deleted Main Data");</script>';
        }
    }
}
